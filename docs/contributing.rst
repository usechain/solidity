...
Othwerise, upon execution the fuzzer halts with an error saying binary is not instrumented:
...
at most one input file per functionality. There is also a tool called ``afl-cmin`` that can trim input files
